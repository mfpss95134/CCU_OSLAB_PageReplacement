...
class RBTree:
    ...
    def fnd(self, frame): #暫時沒用
        current = self.root
        while current != self.nil:
            if frame == current.frame:
                return current.frame
            elif frame < current.frame:
                current = current.left
            else:
                current = current.right
        return None
...
class MyCache:
    ...
    def access(self, frame_number):
        print(f"Accessing Frame:{hex(frame_number)} ...... ", end=f'Time:{int(time.time())}\n')
...
# Example usage
cache = MyCache()
test_sequence = [1, 5, 9, 13, 17, 21, 25, 29, 1, 5, 1, 9, 13, 17, 21, 25, 29, 33, 33, 37, 37, 1,5,1,5,1,5]
